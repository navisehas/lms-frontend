                ...
                {scanResult.data.courses?.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {scanResult.data.courses.map((course) => (
                      <div key={course.course_id} className="bg-white p-4 rounded-xl border border-gray-200 flex items-center justify-between shadow-sm hover:border-indigo-300 transition">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider border ${
                              course.payment_status === 'PAID' || course.is_paid 
                                ? 'bg-green-50 text-green-700 border-green-200' 
                                : 'bg-red-50 text-red-700 border-red-200'
                            }`}>
                              {course.payment_status === 'PAID' || course.is_paid ? <CheckCircle size={10} /> : <AlertTriangle size={10} />}
                              {course.payment_status === 'PAID' || course.is_paid ? 'Paid' : 'Unpaid'}
                            </span>

                          </div>
                          <p className="text-xs text-gray-500 font-mono">{course.course_id}</p>
                        </div>
                        
                        <button 
                          onClick={() => handleMarkAttendance(course.course_id, course.title)}
                          disabled={attendanceStatus?.loading}
                          className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-600 hover:text-white rounded-lg font-bold transition"
                        >

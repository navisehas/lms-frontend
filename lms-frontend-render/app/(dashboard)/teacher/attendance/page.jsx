...
export default function TeacherAttendanceLog() {
  ...
  useEffect(() => {
    ...
    if (authorised) {
      setTeacherId(authorised.user_id);
      fetchAttendance(authorised.user_id);
      fetchMyCourses(authorised.user_id);
    }
  }, [router]);
  ...
  const downloadReportCSV = () => {
    ...
    csvContent += `ENGLISH GATE - Teacher Attendance Report (${reportModal.type}-WISE)\n`;
    ...
    link.setAttribute("download", `My_Classes_Report_${reportModal.type}_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const downloadReportPDF = async () => {
    ...
    doc.text("Teacher Attendance Summary", textStartX, 28);
    ...
    const tableColumn = [groupByText, "Total Classes", "Attended", "Absent", "Rate (%)"];
    ...
    doc.save(`My_Classes_Report_${reportModal.type}_${new Date().toISOString().split('T')[0]}.pdf`);
  ...
  const downloadTableCSV = () => {
    ...
    let csvContent = "data:text/csv;charset=utf-8,";
    csvContent += `ENGLISH GATE - My Classes Attendance Log\n`;
    csvContent += `Generated On: ${new Date().toLocaleString()}\n`;
    ...
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `My_Classes_Attendance_${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const downloadTablePDF = async () => {
    ...
    doc.setTextColor(50, 50, 50);
    doc.setFont("helvetica", "normal");
    doc.text("My Classes Attendance Log", textStartX, 28);
    ...
    doc.save(`My_Classes_Attendance_${new Date().toISOString().split('T')[0]}.pdf`);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 relative pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Class Attendance</h1>
          <p className="text-gray-500 text-sm mt-1">Manage and track attendance for the courses you teach.</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <button 
            onClick={() => setReportModal({ isOpen: true, type: "STUDENT" })}
            className="flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-indigo-200 text-indigo-700 hover:bg-indigo-50 text-sm font-bold rounded-xl shadow-sm transition w-full sm:w-auto"
          >
            <FileText size={18} /> View Reports
          </button>
          <button 
            onClick={() => setAbsentModal({ ...absentModal, isOpen: true })}
            className="flex items-center justify-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl shadow-sm transition w-full sm:w-auto"
          >
      ...
      {/* Filter Section */}
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-4 items-end text-gray-700">
        <div>
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition bg-gray-50" 
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-9 pr-10 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition bg-gray-50" 
            />
            {filterDate && (
              <button 
        ...
        <div>
          ...
          <div className="relative">
            ...
            <select 
              ...
              className="w-full pl-9 pr-8 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none appearance-none bg-gray-50 transition"
            >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
        ...
        {!loading && (
          <div className="bg-indigo-50/50 border-b border-gray-200 p-4 md:p-5 flex flex-col xl:flex-row justify-between gap-6 shrink-0">
            ...
            <div className="flex items-center gap-2 xl:border-l border-gray-200 xl:pl-6 shrink-0">
               <button 
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 hover:bg-gray-100 rounded-xl text-sm font-bold shadow-sm transition w-full sm:w-auto justify-center"
                >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-5 md:p-6 bg-gray-50 border-b border-gray-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4 shrink-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <select 
                  ...
                  className="w-full lg:w-48 p-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-bold text-gray-900 bg-white shadow-sm transition"
                >
            ...
            <div className="p-5 md:p-6 bg-gray-50 border-t border-gray-100 shrink-0">
               <div className="flex flex-col sm:flex-row justify-end gap-3">
                 ...
                 <button onClick={downloadReportCSV} className="w-full sm:w-auto px-6 py-3 bg-white border border-indigo-200 text-indigo-700 hover:bg-indigo-50 rounded-xl font-bold transition flex justify-center items-center gap-2 shadow-sm order-2">
                   <DownloadCloud size={18} /> CSV Export
                 </button>
                 <button onClick={downloadReportPDF} className="w-full sm:w-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold transition flex justify-center items-center gap-2 shadow-sm order-1 sm:order-3">
              ...
              <div className="p-6 space-y-5 overflow-y-auto flex-1">
                ...
                <div>
                  ...
                  >
                    ...
                    {myCourses.map(course => <option key={course.course_id} value={course.course_id}>{course.title}</option>)}
                  </select>
                </div>
                <div>

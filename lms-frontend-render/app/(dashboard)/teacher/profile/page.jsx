...
import { 
  ...
  Briefcase, FileText, Hash, Calendar, Users
} from "lucide-react";
...
export default function TeacherProfile() {
  ...
  const fetchProfile = async (id) => {
    try {
      const res = await authFetch(`${API}/teacher/profile/${id}`);
      if (res.ok) {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      const res = await authFetch(`${API}/teacher/profile/change-password`, {
        method: "PUT",
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 relative pb-12">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <h1 className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Teacher Profile</h1>
        <p className="text-gray-500 text-sm mt-1">View your professional information and manage security settings.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="space-y-6 lg:sticky lg:top-6">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden text-center">
            <div className="h-28 bg-indigo-700 relative">
            ...
            <div className="pt-16 pb-6 px-6">
              ...
              <div className="mt-3 inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-200">
                {user.status || "Active Teacher"}
              </div>
            </div>
          ...
          <button onClick={handleDownloadID} className="w-full flex items-center justify-center gap-2 bg-indigo-700 hover:bg-indigo-800 text-white font-bold py-3.5 rounded-xl transition shadow-md">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 md:p-8">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
              <div className="p-2 bg-indigo-50 text-indigo-700 rounded-lg"><User size={20} /></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Teacher ID</label>
                <div className="relative">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Specialization</label>
                <div className="relative">
                  <Briefcase size={18} className="absolute left-3 top-3 text-gray-400" />
                  <input type="text" className="w-full pl-10 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-gray-700 cursor-not-allowed text-sm font-medium" value={user.specialization || "N/A"} readOnly />
                </div>
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Description / Bio</label>
                <div className="relative">
                  <FileText size={18} className="absolute left-3 top-3 text-gray-400" />
                  <textarea rows="3" className="w-full pl-10 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-gray-700 cursor-not-allowed text-sm font-medium resize-none" value={user.description || "N/A"} readOnly />
                </div>
              </div>
              <div className="md:col-span-2">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 md:p-8">
            <div className="flex justify-between items-center cursor-pointer" onClick={() => setShowPasswordSection(!showPasswordSection)}>
               ...
               <button className="text-sm font-bold text-indigo-700 hover:underline">
      ...
      <div style={{ position: "absolute", top: "-9999px", left: "-9999px" }}>
        <div ref={cardRef} style={{ width: "340px", height: "540px", backgroundColor: "#ffffff", borderRadius: "16px", fontFamily: "sans-serif", position: "relative", overflow: "hidden", boxShadow: "0 4px 6px rgba(0,0,0,0.1)", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "140px", background: "linear-gradient(135deg, #3730a3 0%, #6366f1 100%)", zIndex: 0 }}></div>

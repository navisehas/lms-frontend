...
export default function StudentFeedbackPage() {
  ...
  async function handleSubmit() {
    if (!newMessage.trim()) return showMessage("Please enter a message.", true);
    setSubmitting(true);
    try {
  ...
  async function handleUpdate() {
    if (!editMessage.trim()) return showMessage("Message cannot be empty.", true);
    setSubmitting(true);
    try {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Submit feedback and track responses from admin.
          </p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-blue-100 shadow-sm p-5 mb-6">
          <h2 className="font-semibold text-gray-700 mb-3 flex items-center gap-2">
            <Send size={16} className="text-blue-500" /> Submit New Feedback
          </h2>
          <textarea
            value={newMessage}
            onChange={(e) => setNewMessage(e.target.value)}
            placeholder="Write your feedback, suggestion, or complaint here..."
            className="w-full border border-gray-200 rounded-lg p-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none"
            rows={4}
          />
          <div className="flex justify-end gap-2 mt-3">
            ...
            <button
              ...
              disabled={submitting}
              className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg disabled:opacity-60 transition-colors"
            >
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full border ${stat.color}`}
                    >
                ...
                {isEditing ? (
                  <div>
                    <textarea
                      ...
                      className="w-full border border-blue-300 rounded-lg p-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none"
                      rows={3}
                    />
                ...
                {fb.status === "RESOLVED" && (
                  <div className="mt-3 flex items-center gap-1.5 text-xs text-green-600 bg-green-50 rounded-lg px-3 py-2">
                ...
                {!isEditing && canEdit && (
                  <div className="flex gap-2 mt-3 pt-3 border-t border-gray-50">
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-800 hover:bg-blue-50 px-3 py-1.5 rounded-lg transition-colors"
                    >
                      <Pencil size={13} /> Edit
                    </button>
                    <button
